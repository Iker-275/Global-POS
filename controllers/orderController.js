...
const updateOrder = async (req, res) => {
  try {
    ...
    // 1. DETERMINE DISHES
    // -----------------------------
    const dishes = req.body.dishesOrdered || existing.dishesOrdered;

    // -----------------------------
    // 2. RECALCULATE TOTAL (SOURCE OF TRUTH)
    // -----------------------------
    const total = roundMoney(
      dishes.reduce((sum, d) => sum + (d.totalAmount || 0), 0)
    );

    // -----------------------------
    // 3. HANDLE PAID AMOUNT
    // -----------------------------
    const paid = req.body.paidAmount !== undefined
      ? roundMoney(req.body.paidAmount)
      : roundMoney(existing.paidAmount);
